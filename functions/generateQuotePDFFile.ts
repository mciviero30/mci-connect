...
async function generateQuotePDF(quote) {
  if (!quote) throw new Error('Quote data is missing');
  if (!quote.items || quote.items.length === 0) throw new Error('Quote has no items');
  if (!quote.customer_name) throw new Error('Customer name is required');
