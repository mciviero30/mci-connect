...
export async function generateInvoicePDF(invoice) {
  ...
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(0, 0, 0);
  doc.text(String(formatDateShort(invoice.invoice_date || invoice.created_date)), valueX, rightY, { align: 'right' });
  doc.line(labelX, rightY + 1, valueX, rightY + 1);
  ...
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(0, 0, 0);
  doc.text(String(formatDateShort(invoice.due_date)), valueX, rightY, { align: 'right' });
  doc.line(labelX, rightY + 1, valueX, rightY + 1);
  ...
  const renderTableHeader = (headerY) => {
    for (let i = 0; i < tableGradientSteps; i++) {
      ...
      doc.setFillColor(gray, gray, gray);
      const rectX = margin + (i * contentWidth) / tableGradientSteps;
      const rectWidth = contentWidth / tableGradientSteps + 0.5;
      doc.rect(rectX, headerY, rectWidth, 7, 'F');
    }
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(7);
    doc.setTextColor(255, 255, 255);
