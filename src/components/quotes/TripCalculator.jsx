...
export default function TripCalculator({ jobAddress, selectedTeamIds, onAddAllItems, totalLaborHours }) {
  ...
  const [tripName, setTripName] = useState('Punch Trip');
      ...
      <CardHeader className="py-2 px-3 cursor-pointer" onClick={() => setIsOpen(!isOpen)}>
        <div className="flex items-center justify-between">
          <CardTitle className="flex items-center gap-2 text-sm text-slate-800">
            ...
            {language === 'es' ? 'Calculadora de Viaje' : 'Trip Calculator'}
        ...
        <CardContent className="px-3 pb-3 space-y-3">
          ...
          <div>
            ...
            <Input
              ...
              placeholder="Punch Trip / Field Verification..."
          ...
          {/* Preview of what the item will look like */}
          <div className="p-2 bg-white rounded border border-purple-200 text-[10px]">
            <p className="font-bold text-slate-800 mb-1">{tripName || 'Trip'} — 1 trip — ${grandTotal.toFixed(2)}</p>
          ...
          >
            ...
            <span className="text-[10px]">
              {language === 'es' ? `Agregar "${tripName}" ($${grandTotal.toFixed(2)})` : `Add "${tripName}" ($${grandTotal.toFixed(2)})`}
            </span>
          </Button>
        </CardContent>
      )}
