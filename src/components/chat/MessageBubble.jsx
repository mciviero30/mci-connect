    ...
    <div className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
      <div className={`max-w-[65%] ${
        ...
        {message.reply_to_message && (
          <div className={`px-2.5 pt-1.5 pb-1 border-l-3 ${
            isMe ? 'border-blue-200 bg-blue-500/20' : 'border-slate-300 dark:border-slate-600 bg-slate-100 dark:bg-slate-700'
          }`}>
            <p className={`text-[10px] ${isMe ? 'text-blue-100' : 'text-slate-600 dark:text-slate-400'}`}>
              Replying to {message.reply_to_sender_name}
            </p>
            <p className={`text-xs ${isMe ? 'text-blue-50' : 'text-slate-700 dark:text-slate-300'} truncate`}>
              {message.reply_to_message}
            </p>
          </div>
        )}
        ...
        {!isMe && (
          <div className="px-2.5 pt-1.5 pb-0.5">
            <button 
              ...
              className="text-[10px] font-semibold text-slate-600 dark:text-slate-400 hover:text-[#3B9FF3] dark:hover:text-blue-400 transition-colors cursor-pointer"
            >
              {message.sender_name}
            </button>
          </div>
            ...
            {Object.entries(reactions).map(([reactionName, users]) => {
              ...
              return (
                <button
                  ...
                  className={`px-1 py-0.5 rounded-full text-[10px] flex items-center gap-0.5 transition-all ${
                    iReacted
        ...
        <div className="px-2.5 pb-1 flex items-center justify-between">
          <p className={`text-[10px] ${isMe ? 'text-blue-100' : 'text-slate-500 dark:text-slate-400'}`}>
          ...
          <div className="flex gap-0.5">
            <Button
              ...
              className={`h-5 px-1 ${isMe ? 'text-white hover:bg-blue-600' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700'}`}
            >
            ...
            <div className="flex gap-0.5">
              {REACTIONS.map(reaction => (
                <button
                  ...
                  className="text-base p-0.5 hover:scale-110 transition-transform"
                >
                  {reaction.emoji}
                </button>
              ))}
