...
export async function notifyJobAssignment({ 
  ...
  return createNotification({
    ...
    message: `${assignerName} assigned you to "${jobName}" on ${date} from ${startTime} to ${endTime}`,
    priority: 'medium',
    actionUrl: '/Calendario',
    relatedEntityType: 'job_assignment',
    relatedEntityId: jobId,
    metadata: { jobName, assignerName, date, startTime, endTime }
  });
}
...
export async function notifyScheduleChange({ 
  ...
  const changeMessages = {
    updated: `Your schedule for "${jobName}" on ${date} has been updated from ${oldTime} to ${newTime}`,
    cancelled: `Your assignment to "${jobName}" on ${date} has been cancelled`,
    rescheduled: `"${jobName}" has been rescheduled from ${oldTime} to ${newTime}`
  };
  ...
  return createNotification({
    ...
    message: changeMessages[changeType] || `Your schedule for "${jobName}" has been changed`,
    priority: 'high',
    actionUrl: '/Calendario',
    relatedEntityType: 'schedule',
    metadata: { jobName, changeType, date, oldTime, newTime }
  });
}