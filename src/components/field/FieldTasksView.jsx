...
export default function FieldTasksView({ jobId, tasks, plans }) {
  ...
  const columns = [
    ...
    { id: 'blocked', label: 'Bloqueadas', color: 'red' },
  ];
  ...
  return (
    <div className="p-6 flex flex-col h-full">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <h1 className="text-2xl font-bold text-[#D4C85C]">Tareas</h1>
        <div className="flex items-center gap-3">
          ...
          <Select value={statusFilter} onValueChange={setStatusFilter}>
            ...
            <SelectContent className="bg-slate-800 border-slate-700">
              ...
              <SelectItem value="blocked" className="text-white">Bloqueadas</SelectItem>
            </SelectContent>
          </Select>
          <div className="flex bg-slate-800/50 rounded-lg p-1">
            <button
              onClick={() => setView('kanban')}
              className={`p-2 rounded ${view === 'kanban' ? 'bg-slate-700 text-white' : 'text-slate-400'}`}
            >
              <LayoutGrid className="w-4 h-4" />
            </button>
            <button
              onClick={() => setView('list')}
              className={`p-2 rounded ${view === 'list' ? 'bg-slate-700 text-white' : 'text-slate-400'}`}
            >
        ...
        <div className="flex-1 grid grid-cols-1 md:grid-cols-4 gap-4 overflow-x-auto">
          {columns.map((column) => {
            ...
            return (
              <div 
                key={column.id}
                className="bg-slate-800/30 rounded-xl p-4 min-h-[400px]"
                onDrop={(e) => handleDrop(e, column.id)}
                onDragOver={handleDragOver}
              >
                <div className="flex items-center justify-between mb-4">
                  <h3 className={`font-semibold text-${column.color}-400`}>{column.label}</h3>
                  <Badge className={`bg-${column.color}-500/20 text-${column.color}-400 border-${column.color}-500/30`}>
                    {columnTasks.length}
                  </Badge>
                </div>
                <div className="space-y-3">
                  {columnTasks.map((task) => (
                    <div
                      ...
                      className="bg-slate-900/50 border border-slate-700/50 rounded-lg p-3 cursor-pointer hover:border-amber-500/50 transition-all"
                    >
      ...
      {/* List View */}
      {view === 'list' && (
        <div className="flex-1 bg-slate-800/30 rounded-xl overflow-hidden">
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-700">
                ...
                <th className="text-left p-4 text-slate-400 font-medium">Prioridad</th>
                <th className="text-left p-4 text-slate-400 font-medium">Categoría</th>
                <th className="text-left p-4 text-slate-400 font-medium">Asignado</th>
              </tr>
            </thead>
            <tbody>
