            ...
            <div className="space-y-2">
              {overdueTasks.slice(0, 3).map(task => (
                <button
                  ...
                  className="w-full flex items-center justify-between p-2 bg-white dark:bg-slate-800/50 rounded-lg hover:bg-red-100 dark:hover:bg-red-500/20 transition-colors group"
                >
            ...
            <div className="space-y-2">
              {dueTodayTasks.slice(0, 3).map(task => (
                <button
                  key={task.id}
                  onClick={() => onTaskClick?.(task)}
                  className="w-full flex items-center justify-between p-2 bg-white dark:bg-slate-800/50 rounded-lg hover:bg-amber-100 dark:hover:bg-amber-500/20 transition-colors group"
                >
                  <div className="flex items-center gap-2 min-w-0">
      ...
      {(urgentTasks.length > 0 || blockedTasks.length > 0) && (
        <div className="grid grid-cols-2 gap-3">
          ...
          {urgentTasks.length > 0 && (
            <div className="bg-orange-50 dark:bg-orange-500/10 border border-orange-200 dark:border-orange-500/30 rounded-xl p-3">
              ...
              <div className="space-y-1">
                {urgentTasks.slice(0, 2).map(task => (
                  <button
                    key={task.id}
                    onClick={() => onTaskClick?.(task)}
                    className="w-full text-left text-xs text-slate-700 dark:text-slate-300 truncate hover:text-orange-600 dark:hover:text-orange-400"
                  >
                    • {task.title}
                  </button>
                ))}
          ...
          {blockedTasks.length > 0 && (
            <div className="bg-slate-100 dark:bg-slate-500/10 border border-slate-200 dark:border-slate-500/30 rounded-xl p-3">
              <div className="flex items-center gap-2 mb-2">
              ...
              <div className="space-y-1">
                {blockedTasks.slice(0, 2).map(task => (
                  <button
                    key={task.id}
                    onClick={() => onTaskClick?.(task)}
                    className="w-full text-left text-xs text-slate-600 dark:text-slate-400 truncate hover:text-slate-900 dark:hover:text-white"
                  >
                    • {task.title}
                  </button>
                ))}
